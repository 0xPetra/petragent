...
Auto-generated by the `heartbeat` skill on every run. Derive the cadence phrase and staleness threshold from heartbeat's own entry in `aeon.yml` — do **not** hardcode `3× daily at 08:00 / 14:00 / 20:00 UTC` (the actual schedule is operator-configurable and varies across forks). Render as: `(<cadence> — e.g. "daily at 14:00 UTC" / "twice daily at 08:00 / 20:00 UTC" / "every 4h"). If the Updated timestamp is more than ~<threshold> stale, the agent is not running.` Set `<threshold>` to ~2× the cadence interval (≈26h for daily, ≈16h for 2×/day, ≈9h for 3×/day, ≈9h for every-4h).
